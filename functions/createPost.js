    ...
    async (ctx) => {
      try {
        ...
        if (post.photos.length === 1) {
          await ctx.replyWithPhoto(post.photos[0], {
            caption: postMessageBuilder(post),
            parse_mode: 'HTML',
            disable_web_page_preview: true
          });
        } else if (post.photos.length > 1) {
          await ctx.replyWithMediaGroup(post.photos.slice(0, -1).map(photo => ({type: "photo", media: photo})));
          await ctx.replyWithPhoto(post.photos[post.photos.length - 1], {
            caption: postMessageBuilder(post),
            parse_mode: 'HTML',
            disable_web_page_preview: true
          });
    ...
    async (ctx) => {
      try {
        ...
        if (ctx.callbackQuery.data === '1') {
          ...
          try {
            if (post.photos.length === 1) {
              await bot.telegram.sendPhoto(process.env.CHAT_ID, post.photos[0], {
                caption: postMessageBuilder(post),
                parse_mode: 'HTML',
                disable_web_page_preview: true
              });
            } else if (post.photos.length > 1) {
              await bot.telegram.sendMediaGroup(process.env.CHAT_ID, post.photos.slice(0, -1).map(photo => ({
                type: "photo",
                media: photo
              })));
              await bot.telegram.sendPhoto(process.env.CHAT_ID, post.photos[post.photos.length - 1], {
                caption: postMessageBuilder(post),
                parse_mode: 'HTML',
                disable_web_page_preview: true
              });
